...
TLSN_EXAMPLE_DIR = TLSN_PROJECT_ROOT / 'tlsn' / 'examples' / 'simple'
PARTY_DATA_DIR = BENCHMARK_DIR / 'Player-Data'
...
def prepare_data():
    proofs = proofs_for_different_number_of_digits
...
def generate_tlsn_proofs() -> list[TLSNProof]:
    ...
    for party_index in range(NUM_PARTIES):
        ...
        try:
            res = subprocess.run(
                f"cd {TLSN_EXAMPLE_DIR} && {CMD_GEN_TLSN_PROOF} {party_index} {proof_file}",
