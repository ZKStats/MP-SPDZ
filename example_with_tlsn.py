    ...
    def computation():
        ...
        def calculate_data_commitment(followers: sint, delta: sbitvec, encoding: list[sbitvec], nonce: sbitvec):
            ...
            for i in range(NUM_REDACTED_BYTES):
                divisor = sint(10**(NUM_REDACTED_BYTES-1-i))
                # curr_digit = current digit tracing from MSB to LSB
                # Since int_div() requires bit of inputs, bit of inputs <= log_2(10**NUM_REDACTED_BYTES) < 4* NUM_REDACTED_BYTES
                curr_digit = number.int_div(divisor, 4*NUM_REDACTED_BYTES)
                followers_bits_list += [sint(ele) for ele in sbitvec(curr_digit+ASCII_BASE, 8).v]
                number = number.int_mod(divisor, 4*NUM_REDACTED_BYTES)
            ...
            concat = nonce.bit_decompose()+ sbitvec(sint(NUM_REDACTED_BYTES), 8).bit_decompose()            
            for i in range(len(encoding)):
                if i%8 ==0:
