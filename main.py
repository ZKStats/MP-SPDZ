...
from mpcstats_lib import print_data, mean, join, median, covariance, correlation, where, geometric_mean
...
COMPILE_PY = PROJECT_ROOT / 'compile.py' 
...
def compile_run(computation):
    ...
    cmd = f"PLAYERS={NUM_PARTIES} {str(LOCAL_EXECUTION_EXE)} {PROGRAM_NAME}"
    print(f"!@# {cmd=}")
    code = os.system(cmd)
    if code != 0:
...
if __name__ == "__main__":
    ...
    def computation():
        ...
        matrix_0 = sfix.input_tensor_from(0, [2,4])
        ...
        matrix_1 = sfix.input_tensor_from(1, [2,4])
        ...
        # ]
        new_data = join(matrix_0, matrix_1, 0, 0, sfix)
        # print_ln("new_data:")
        # print_data(new_data)
        result = mean(new_data[3])
        ...
        print_ln('Found mean of joined tensor_1, value is : %s.', result.reveal())
